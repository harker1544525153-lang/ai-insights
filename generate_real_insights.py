...
def fetch_real_rss_data():
    ...
    # 阿里云微信公众号数据源
    aliyun_rss_url = "https://wechatrss.waytomaster.com/api/rss/MzA4NjI4MzM4MQ==?token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIzMyIsInR5cGUiOiJyc3MifQ.jD9i4ptS7kVD4xAlCly_pQN4E2DMnk1h0brfzOJA1lM"
    
    try:
        # 获取阿里云微信公众号数据
        print("📡 获取阿里云微信公众号数据...")
        feed = feedparser.parse(aliyun_rss_url)
        ...
        if feed.entries:
            ...
            for i, entry in enumerate(feed.entries[:3]):
                insight = {
                    ...
                    'category': '云计算',
                    'source': '阿里云（微信公众号）',
                    'publish_time': entry.published if hasattr(entry, 'published') else '2026-03-07',
                    'summary': generate_summary_from_title(entry.title),
                    'comment': generate_comment_from_title(entry.title),
                    'link': entry.link
                }
                real_insights.append(insight)
                print(f"✅ 获取阿里云简讯: {entry.title}")
        
    except Exception as e:
        print(f"❌ 获取阿里云数据失败: {e}")
        # 使用备用数据
        backup_aliyun_articles = [
            {
        ...
        for i, article in enumerate(backup_aliyun_articles):
            insight = {
                'number': len(real_insights) + 1,
                'title': article['title'],
                'category': '云计算',
...
def generate_summary_from_title(title):
    ...
    if '阿里云' in title:
        if 'Coding Plan' in title:
            return '阿里云Coding Plan订阅量激增，为保障用户体验，从3月4日开始对首购优惠活动进行阶段性调整。Lite版套餐7.9元首购优惠和Pro版套餐39.9元首购优惠，每日9:30-11:30、14:30-16:30两个时间段限量供应。'
        elif '百炼' in title:
