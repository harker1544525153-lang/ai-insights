...
# -*- coding: utf-8 -*-
"""
数据源统计管理器 - 确保resultAI表格仅呈现成功获取的数据源信息
"""
...
class ResultAIManager:
    ...
    def load_ai_sources(self):
        ...
        try:
            # 读取Excel文件
            df = pd.read_excel(self.ai_sources_file)
            return df.to_dict('records')
        except Exception as e:
            print(f"❌ 加载AI数据源配置失败: {e}")
            # 返回默认配置
    ...
    def generate_result_file(self):
        ...
        try:
            ...
            # 创建DataFrame
            df = pd.DataFrame(self.sources_data)
            
            # 按优先级排序
            df = df.sort_values('priority', ascending=False)
            
            # 保存为CSV文件
            df.to_csv(self.result_file, index=False, encoding='utf-8')
            print(f"✅ 数据源统计文件已生成: {self.result_file}")
            
            # 同时尝试保存为Excel格式
            try:
                df.to_excel('source/resultAI.xlsx', index=False)
                print("✅ Excel格式文件已生成: source/resultAI.xlsx")
            except Exception as e:
                print(f"⚠️ 无法生成Excel文件: {e}")
