...
class PriorityDataManager:
    ...
    def refine_title(self, source_name):
        """提炼标题"""
    ...
    def generate_priority_insights(self):
        """生成按发布时间排序的简讯内容"""
        ...
        for i, (_, source) in enumerate(real_sources.iterrows(), 1):
            ...
            publish_time_str = self.get_real_publish_time(source_name, link)
            publish_time = self.parse_publish_time(publish_time_str)
            
            insight = {
                'number': i,
                'title': self.refine_title(source_name),
                'category': category,
                'source': source_name,
                'publish_time': publish_time_str,
                'publish_datetime': publish_time,
                'summary': self.generate_objective_summary(source_name, category),
                'comment': self.generate_cloud_perspective_comment(source_name, category),
                'link': link
            }
            insights.append(insight)
        
        # 按发布时间排序（最近的时间优先级最高）
        insights.sort(key=lambda x: x['publish_datetime'], reverse=True)
...
# 使用示例
def main():
    manager = PriorityDataManager()
    
    # 生成按优先级排序的简讯内容
    insights = manager.generate_priority_insights()
