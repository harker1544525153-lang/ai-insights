...
# -*- coding: utf-8 -*-
"""
最终版AI简讯生成器 - 从AI_sources.xlsx获取数据源并生成今日最新简讯
"""
...
class FinalAIInsightsGenerator:
    """最终版AI简讯生成器"""
    ...
    def fetch_rss_data(self, rss_url, source_name):
        ...
        try:
            ...
            articles = []
            for entry in feed.entries[:3]:  # 只取最新3条
                article = {
                    'title': entry.title,
                    'link': entry.link,
...
'''}
        ...
        # 添加分享标题
        for i, insight in enumerate(self.insights, 1):
            html_content += f'                \"{i}. {insight[\"title\"]}\",\n'
        
        html_content += '''            ];
            
            const shareText = insightTitles.join('\\n') + '\\n\\n更多详情请访问：' + window.location.href;
...
def main():
    ...
    generator = FinalAIInsightsGenerator()
    generator.run()
