...
def fix_title_style():
    ...
    try:
        ...
        for i, insight in enumerate(insights):
            if re.search(r'^\d+\.\s*', insight['title']):
                print(f"⚠️  标题 {i+1} 仍包含序号: {insight['title']}")
                # 移除序号
                insight['title'] = re.sub(r'^\d+\.\s*', '', insight['title'])
                print(f"✅  修复后标题: {insight['title']}")
...
def verify_fix_results():
    ...
    # 检查修复的关键问题
    fix_checks = {
        "序号不重复": "insight-num" in content and not re.search(r'<div class="insight-title">\d+\.', content),
